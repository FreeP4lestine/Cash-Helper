    ...
    Enter::
        ...
        If (!Selling) {
            ...
            If FileExist("Dump\Last.sell") {
                LastOBJ := FileOpen("Dump\Last.sell", "r")
                While (!LastOBJ.AtEOF()) {
                    LastOBJArr[++Ind] := "" Trim(LastOBJ.ReadLine(), "`n") ""
                }
                LastOBJ.Close()
            }
            I := Sum := Cost := 0
            ...
            LastArr := {}, C := 0
            Loop {
                If !LastArr.HasKey("" LastOBJArr[Ind] "") {
                    LastArr["" LastOBJArr[Ind] ""] := ""
                    ++C
                }
            } Until (--Ind <= 0) || (C = 10)
            
            LastOBJ := FileOpen("Dump\Last.sell", "w")
            For Each, Line in LastArr {
                LastOBJ.Write(Each "`n")
            }
            LastOBJ.Close()
